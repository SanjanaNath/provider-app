...
class ErrorHandler {
  static Future<dynamic> processResponse({
    ...
    try {
      ...
      message = json['message'] ?? getErrorMessage(statusCode);
    } catch (e) {
  ...
  static reAuth(String message) {
    ...
    if (context != null) {
      showError();
      // context.read<AuthControllers>().logOut(context: context, message: message, color: Colors.red);
    }
  }
  ...
  static catchError(Object e, StackTrace s, bool? showError) {
    ...
    if (e is ApiException) {
      ...
      ApiException(message: e.description ?? 'Camera Exception', originalException: e, stackTrace: s)
          .errorHandle(showError: showError);
